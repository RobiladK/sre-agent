...
try {
    & (Join-Path $PSScriptRoot 'fix-bad-deploy.ps1') -ResourceGroup $ctx.ResourceGroup -ClusterName $ctx.ClusterName -Namespace $Namespace
...
try {
    & (Join-Path $PSScriptRoot 'fix-db-perf.ps1') -ResourceGroup $ctx.ResourceGroup -ClusterName $ctx.ClusterName -Namespace $Namespace
